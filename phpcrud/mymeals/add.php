<?php
include_once('Database.php');

        $db = new Database();
        $name =  @$_POST['name'];
        $mail = @$_POST['mail'];
        
        
        function validation($data){
        $data1 = trim($data);
        $data2 = stripcslashes($data1);
        $data3 = htmlspecialchars($data2);
        return $data3; // here i return this $data variable so we can use this.
        }
       function sqlonlemeforSerch($data)
        {
        $bad = array("content-type","'","bcc:","to:","cc:","href","* from","* FROM","*from","*FROM","SET","set","update","UPDATE","updateset","UPDATESET","UPDATE SET","DELETE","delete","*","SELECT*FROM","s3","S3","%7C","%2B");
        $islem = str_replace($bad,"",$data);
        
        return $islem;
        }
      
        if (!isset($name)  || $name == NULL ||
         !isset($mail)  || $mail == NULL ) {
         exit();

        }
        else 
        {
            $clName = $db->link->real_escape_string($name);
            $clEmail = $db->link->real_escape_string($mail);
            $clearName = sqlonlemeforSerch(validation($clName));
            $clearEmail = sqlonlemeforSerch(validation($clEmail));
                $inserted = $db->insert("INSERT INTO userms(name, mail)
                VALUES ('$clearName','$clearEmail')"); 
                if($inserted)
                {
                    echo 'Success';
                    $db ->link->close();
                }
          
          
          return;
    
        }
        
  
?>